...
module.exports = {
	DATE_HEADER_SEMANTIC_LABELS:config.has('DATE_HEADER_SEMANTIC_LABELS') ? config.get('DATE_HEADER_SEMANTIC_LABELS'):[
	...
	DATE_HEADER_PATTERNS:config.has('DATE_HEADER_PATTERNS') ? arrayOfRegexes(config.get('DATE_HEADER_PATTERNS')):[
	...
	INT_HEADER_SEMANTIC_LABELS: config.has('INT_HEADER_SEMANTIC_LABELS') ? config.get('INT_HEADER_SEMANTIC_LABELS'):[
	...
	INT_HEADER_PATTERNS: config.has('INT_HEADER_PATTERNS') ? arrayOfRegexes(config.get('INT_HEADER_PATTERNS')):[
	/.*PHONE.*|.*PH.?NUM.*/i,
	/^PHN$/i,
	/^PEN$/i
	],
	STRING_HEADER_SEMANTIC_LABELS: config.has('STRING_HEADER_SEMANTIC_LABELS') ? config.get('STRING_HEADER_SEMANTIC_LABELS') :[
	{name:'Phone number',rdfType:'https://schema.org/telephone',var_class:'direct_identifier'},
	...
	STRING_HEADER_PATTERNS: config.has('STRING_HEADER_PATTERNS') ? arrayOfRegexes(config.get('STRING_HEADER_PATTERNS')):[
	/.*PHONE.*|.*PH.?NUM.*/,
	...
	STRING_VALUE_SEMANTIC_LABELS: config.has('STRING_VALUE_SEMANTIC_LABELS') ? config.get('STRING_VALUE_SEMANTIC_LABELS'):[
	...
	STRING_VALUE_PATTERNS: config.has('STRING_VALUE_PATTERNS') ? arrayOfRegexes(config.get('STRING_VALUE_PATTERNS')):[
	...
	INT_VALUE_SEMANTIC_LABELS: config.has('INT_VALUE_SEMANTIC_LABELS') ? config.get('INT_VALUE_SEMANTIC_LABELS'):[
	{name:'PHN',var_class:'direct_identifier',th:1},
	{name:'PEN',var_class:'direct_identifier',th:1}
	],
	INT_VALUE_PATTERNS: config.has('INT_VALUE_PATTERNS') ? arrayOfRegexes(config.get('INT_VALUE_PATTERNS')):[
	/^9\d{9}$/,
	/^\d{9}$/
	]
};